...
function proportionArrayModification(asset,n,i){
    ...
    proportionArray[i]["earnings"] = proportionArray[i]["earnings"] + profitObjectInDollar[asset] * remainingTime;
    maxProfit(n,i);
}
...
function maxProfit(n,i){
    if(n>15){
        ...
        tempProportionOject = proportionArray[i] ? {...proportionArray[i]} : {...proportionOject};
        possibleAssets.map((asset,index)=>{
            let k = i+index;
            let remainingTime = n===15 ? 15 - timeTakenObject[asset] : 7-timeTakenObject[asset];
            let time = remainingTime;
            !proportionArray[k] && proportionArray.push({...tempProportionOject});
            proportionArray[k]["earnings"] = proportionArray[k]["earnings"] + profitObjectInDollar[asset] * remainingTime;
...
let outputArray = proportionArray.filter((value)=>value.earnings===maximumEarnings);

outputArray.map((value,index)=>{
    console.log(`${index+1}. T: ${value.T} P: ${value.P} C: ${value.C}`);
});

// console.log("proportionArray.............",proportionArray,outputArray);